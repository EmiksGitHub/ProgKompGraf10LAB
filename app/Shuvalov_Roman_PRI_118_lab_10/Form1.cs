        ...
        static void CalculateImage(object Settings)
        {
            ...
            for (int ax = thisThreadSettings._FromImageH; ax < thisThreadSettings._ToImageH; ax++)
            {
                for (int bx = 0; bx < thisThreadSettings._ImageW; bx++)
                {
                    ...
                    X = x;
                    Y = y;
                    var tempx = mode_[thisThreadSettings.code_mode, 0];
                    var tempy = mode_[thisThreadSettings.code_mode, 1];
                    double ix = 0, iy = 0, n = 0;
                    // выполнение итерации
                    while ((ix * ix + iy * iy < 64))
                    {
                        if (n > 64) break;
                        ...
                        /*else
                        {
                            ...
                            ix = X * X * X - 3 * X * Y * Y + tempx / (tempx * tempx + tempy * tempy);
                            iy = 3 * X * X * Y - Y * Y * Y - tempy / (tempx * tempx + tempy * tempy);
                        }*/
                        else
                        {
                            // иначе используем значения из массива _mode используя индекс comboBox1,
                            // переданный в поток через thisThreadSettings
                            ix = X * X * X * X * X - 10 * X * X * X * Y * Y + 5 * X * Y * Y * Y * Y + tempx;
                            iy = 5 * X*X*X*X*Y - 10*X*X*Y*Y*Y + Y*Y*Y*Y*Y + tempy;
                        }
